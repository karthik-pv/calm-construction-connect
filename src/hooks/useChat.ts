...
const fetchChatPartners = async (userId: string): Promise<ChatPartner[]> => {
  ...
  const { data: messagePartners, error: messageError } = await supabase.rpc('get_chat_partners', { current_user_id: userId });
...
const fetchMessages = async (userId: string, partnerId: string): Promise<ChatMessage[]> => {
...
export function useChatMessages(partnerId: string | null) {
  ...
  return useQuery<ChatMessage[], Error>({
    queryKey: queryKey,
    queryFn: () => {
      if (!user?.id || !partnerId) throw new Error("User or partner ID missing");
...
export function useChatSubscription(partnerId: string | null) {
  ...
  useEffect(() => {
    ...
    if (channel?.topic === `realtime:public:chat_messages:partner=${partnerId}`) {
    ...
    const newChannel = supabase.channel(`chat_messages_user_${user.id}`)
      .on<ChatMessage>(
        'postgres_changes',
        {
          event: 'INSERT',
        ...
        async (payload) => {
           ...
           if (payload.new.receiver_id === partnerId) {
             ...
             await queryClient.invalidateQueries({ queryKey: ['chatMessages', user.id, partnerId] });
             // Could also optimistically update here
           }
           // Also invalidate chat partners if it's a new chat
           await queryClient.invalidateQueries({ queryKey: ['chatPartners', user?.id] });
        }
      )
  ...
  const fetchConversations = async (): Promise<Conversation[]> => {
    ...
    return conversations
      ...
      .sort((a, b) => {
        ...
        if (aTime === 'Yesterday' && !['Just now', 'Today'].includes(bTime)) return -1;
        if (bTime === 'Yesterday' && !['Just now', 'Today'].includes(aTime)) return 1;
...
export const useMarkMessagesAsRead = () => {
  ...
  return useMutation({
    mutationFn: async (senderId: string) => {
      if (!profile?.id) throw new Error('You must be logged in to mark messages as read');
